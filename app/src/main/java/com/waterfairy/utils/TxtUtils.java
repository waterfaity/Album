...
public class TxtUtils {
     ...
     * @return
     */
    private String getCurrentWord(String refText, String word, int position) {
        int pun = 0;
        boolean isRight = false;//正确位置 之后的标点符号加到word 后面
        for (int i = 0; i < refText.length(); i++) {
            char charAt = refText.charAt(i);
            if (TxtUtils.isChineseByBlock(charAt)) {//中文
                if (isRight) {
                    return word;
                }
                if (pun + position == i && word.charAt(0) == charAt) {
                    isRight = true;
                }
            } else {
                //非中文字符
                pun++;
                if (isRight) {
                    word += (Character.toString(charAt));
                }
            }
        }
        return word;
